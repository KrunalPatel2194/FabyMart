...
{
    public class tblNewArrival : _tblNewArrival
    {
        public tblNewArrival()
        {
        ...
        public DataTable LoadGridData(string strColumnName, string strColumnValue)
        {
            ...
            string StrQuery = "select tblNewArrival.appNewArrivalID,tblNewArrival.appProductID,tblNewArrival.appIsActive,tblNewArrival.appDisplayOrder,tblProduct.appProductName,tblColor.appColorName,tblColor.appColorCode,tblColor.appColorImage,tblProductImage.appThumbImage,tblProductImage.appNormalImage,tblProductDetail.appPrice ";
            StrQuery += " from tblNewArrival";
            StrQuery += " inner join tblProduct on tblProduct.appProductID=tblNewArrival.appProductID ";
            ...
            StrQuery += " order by tblNewArrival.appDisplayOrder ";
            base.LoadFromRawSql(StrQuery);
            return base.DefaultView.Table;
        }

        public DataTable LoadNewArrival(string strTopRecord = "")
        {
        ...
        public DataTable LoadNewArrivalList(string strCategoryId, string strSubCategoryId, string strColorId)
        {
            ...
            StrQuery += " and vw_SerachProduct.appProductID not in ( Select tblNewArrival.appProductID From tblNewArrival)";
        ...
        public DataTable LoadAllProductByPaging(string strPageSize, string strPageIndex, string strProperty, string strColor, string strSize, string strOrderBy, string strMinPrice, string strMaxPrice, string strRate)
        {
            ...
            StrQuery += " )AS RowNumber,CASE WHEN tblDeal.appDiscountPer IS NULL THEN tblProductDetail.appPrice ELSE CAST((isnull(tblProductDetail.appPrice, 0) - ((isnull(tblProductDetail.appPrice, 0) * tblDeal.appDiscountPer) / 100)) AS Decimal(10, 2)) END AS AppPrice,tblProductColor.appProductColorID,tblProductDetail.AppSizeID, tblProductDetail.AppMRP,tblNewArrival.appNewArrivalID,tblNewArrival.appProductID,tblNewArrival.appIsActive,tblNewArrival.appDisplayOrder,tblProduct.appProductName,tblColor.appColorName,tblColor.appColorCode,tblColor.appColorImage,tblProductImage.appSmallImage,tblProductImage.appNormalImage,tblProductImage.appLargeImage,tblProductDetail.appProductDetailID,case when tblDeal.appDiscountPer is null then Cast(ROUND((100-(isnull(dbo.tblProductDetail.appPrice,0)*100)/isnull(dbo.tblProductDetail.appMRP,0)),0) as Decimal(2)) else Cast(ROUND((100-(isnull( cast((isnull(tblProductDetail.appPrice,0)-((isnull(tblProductDetail.appPrice,0)*tblDeal.appDiscountPer)/100)) as Decimal(10,2)),0)*100)/isnull(dbo.tblProductDetail.appMRP,0)),0) as decimal(2,0))  end appOff  ";
            StrQuery += "     from tblNewArrival ";
            StrQuery += "   inner join tblProduct on tblProduct.appProductID=tblNewArrival.appProductID and tblNewArrival.appIsActive=1  ";
            StrQuery += "    inner join tblProductColor on tblProductColor.appProductID=tblProduct.appProductID  ";
            if (strColor == "")
            ...
            if (strProperty != "")
            {
                StrQuery += " INNER JOIN (Select appProductID From tblProductProperty Where tblProductProperty.appPropertyPreValueID in (" + strProperty + ") group by appProductID) tblProductProperty On tblProductProperty.appProductId=tblNewArrival.appProductId";
            }
            StrQuery += " Where 1=1 ";
