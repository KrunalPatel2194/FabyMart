...
public class clsCommon : MyGeneration.dOOdads.SqlClientEntity
{
    ...
    public void SendConfirmationMail(string strTO, string strSubject, string strBody, Enums.Enum_Confirmation_Mail_type strConfirmationMailType, FileUpload objFileUpload = null)
    {
        try
        {
            string strFrom = ""; ;
            if (Convert.ToInt32(strConfirmationMailType) == Convert.ToInt32(Enums.Enum_Confirmation_Mail_type.order))
            {
                strFrom = "order@fabymart.com";
            }
            if (Convert.ToInt32(strConfirmationMailType) == Convert.ToInt32(Enums.Enum_Confirmation_Mail_type.register))
            {
                strFrom = "register@fabymart.com";
            }
            if (Convert.ToInt32(strConfirmationMailType) == Convert.ToInt32(Enums.Enum_Confirmation_Mail_type.shipping))
            {
                strFrom = "shipping@fabymart.com";
            }
            MailMessage mail = new MailMessage(strFrom, strTO, strSubject, strBody);
            mail.IsBodyHtml = true;
            SmtpClient smtp = new SmtpClient();
            smtp.Host = "mail.fabymart.com";
            //smtp.Port = Convert.ToInt32(objSettings.AppPortNumber.ToString());
            // smtp.EnableSsl = true;
            smtp.UseDefaultCredentials = false;
            smtp.Credentials = new System.Net.NetworkCredential(strFrom, "faby@123");
            smtp.Send(mail);
            smtp = null;
            mail = null;
        }
    ...
    public void SendOrderSMS(string strSMSText, string StrMobileNo)
    {

        //strSMSText = HttpUtility.UrlEncode(strSMSText);
        //string StrSMSurl = appFunctions.strSMSURL;
        //if (StrSMSurl.Contains("`mobileno`") && StrSMSurl.Contains("`smstext`"))
        //{
        //    StrSMSurl = StrSMSurl.Replace("`mobileno`", StrMobileNo);
        //    StrSMSurl = StrSMSurl.Replace("`smstext`", strSMSText);
        //    if (SendSMS(StrSMSurl))
        //    {
        //        PageBase objPageBase = new PageBase();
        //        tblSentSMS objSMSHistory = new tblSentSMS();
        //        objSMSHistory.AddNew();
        //        objSMSHistory.AppMobileNo = StrMobileNo;
        //        objSMSHistory.s_AppSentDate = objPageBase.GetCurrentDateTime();
        //        objSMSHistory.AppText = strSMSText;
        //        objSMSHistory.AppSentBy = 1;
        //        objSMSHistory.Save();
        //        objSMSHistory = null;
        //        objPageBase = null;
        //    }
    ...
    public void AddToCart(string strProductId, string strProductColorId, string strProductDetailID, int intQty, string strCouponDiscount = "0")
    {
        ...
        if (dtCart.Rows.Count > 0)
        {
            ...
            if (dr.Length > 0)
            {
                ...
                if (objProductDetail.LoadByPrimaryKey(Convert.ToInt32(dr[0]["appProductDetailID"].ToString())))
                {
                    if (objProductDetail.AppQuantity >= (iQty + intQty))
                    {
                    ...
                    {
                        dr[0]["appQty"] = objProductDetail.AppQuantity;
                    }
                    ...
                    dr[0]["appDiscountPrice"] = Math.Round(appTotalDiscount);
                    dr[0]["appTotalPrice"] = Math.Round(((Convert.ToDecimal(dr[0]["appRealPrice"].ToString()) * Convert.ToDecimal(dr[0]["appQty"])) - appTotalDiscount)).ToString();
                    dr[0][tblProductDetail.ColumnNames.AppPrice] = (Convert.ToInt32(dr[0]["appRealPrice"].ToString()) * Convert.ToInt32(dr[0]["appQty"])).ToString();
        ...
        if (IsNew)
        {
            ...
            if (objDataTable.Rows.Count > 0)
            {
                ...
                //    {
                //        int iQty = intQty;
                //        decimal appRealDiscountPrice = Convert.ToDecimal(strCouponDiscount);
                //        decimal appTotalDiscount = Convert.ToDecimal(Convert.ToDecimal(dr[0]["appQty"]) * Convert.ToDecimal(strCouponDiscount));
